...
add_action( 'widgets_init', 'repick_tabs_load_widget' );

function repick_tabs_load_widget() {
	register_widget( 'repick_tabs_widget' );
}

class repick_tabs_widget extends WP_Widget {

	/**
	 * Widget setup.
	 */
	function repick_tabs_widget() {
		/* Widget settings. */
		...
		/* Create the widget. */
		$this->WP_Widget( 'rehub_latest_tabs_widget', __('RePick: Tabs', 'rehub_framework'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if( empty($instance['basedby']) ) {$basedby = 'hot';}
    ...
    <div class="color_sidebar<?php if ($color == 'dark') :?> dark_sidebar<?php endif ;?>">
       <div class="tabs-item clearfix">
   			<?php if ($tabs1 == 'popular') :?>
            	<?php rehub_most_popular_widget_block($basedby, $sortby, $number);?>
       ...
       <div class="tabs-item">
          	<?php if ($tabs2 == 'popular') :?>
            	<?php rehub_most_popular_widget_block($basedbysec, $sortbysec, $number);?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'tabs1' => 'popular', 'tabs2' => 'comments', 'sortby' => 'all_time', 'sortbysec' => 'all_time', 'basedby' => 'hot', 'basedbysec' => 'views', 'titlefirst' => __('Popular', 'rehub_framework'),  'titlesecond' => __('Comments', 'rehub_framework'), 'number' => 5);
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e('Enter how many posts to show', 'rehub_framework'); ?></label>
