...
function rehub_framework_register_scripts() {
	...
	wp_register_script('tablesorter', get_template_directory_uri() . '/js/jquery.tablesorter.min.js', array('jquery'), '2.0.5'); // table sorter
	wp_register_script('touchswipe', get_template_directory_uri() . '/js/jquery.touchSwipe.min.js', array('jquery'), '2.0.5'); // table sorter
...
function rehub_enqueue_scripts() {
    ...
    if (class_exists('\Keywordrush\AffiliateEgg\AffiliateEgg')) {wp_enqueue_style('eggrehub');}	
	...
	wp_localize_script( 'affegg_coupons', 'translation', $affcoupons_array );
...
function rehub_formats_icons($editor='no')
{
	...
	elseif(vp_metabox('rehub_post.rehub_framework_post_type') == 'review') {
		...
		if ( !empty($offer_price) && !empty($offer_price_old) ) {
			...
			if ($offer_priceold !='0' && is_numeric($offer_priceold)) {
...
if (!function_exists('rehub_get_site_favicon')) {
    ...
    function rehub_get_site_favicon($url) {
        ...
        $shop_str = '<img src="http://www.google.com/s2/favicons?domain=http://' . $shop . '"> ';
        $shop_str .= $shop;
        return $shop_str;
    }
}
